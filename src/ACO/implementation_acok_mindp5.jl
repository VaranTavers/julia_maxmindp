...
function calculate_probabilities(inner::ACOKInner, vars::ACOKSettings)
	graph, n, η, τ = spread(inner)
	# graph.weights[i,j] * 
	p = [ (τ[j]^vars.α * η[j]^vars.β) for j in 1:n]
...
function generate_s(inner::ACOKInner, vars::ACOKSettings, i)
	...
	for i in 2:vars.k
		points[i] = sample(calculate_probabilities(inner, vars))
...
function ACOK(graph, vars::ACOKSettings, η)
	...
	η = [n - 1 for i in 1:nv(graph)] ./ sum(η, dims=2)
