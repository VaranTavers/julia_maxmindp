...
function calculate_probabilities(inner::ACOKInner, points, i, vars::ACOKSettings)
	...
	mindps = map(x -> calculate_mindist(graph, vector_with(points[1:(i-1)], x), η), 1:n)
	for i in 1:(i-1)
		mindps[points[i]] = 0
	end
	mindps /= 1000
	...
	# graph.weights[i,j] * 
	p = [ (τ[j]^vars.α * mindps[j]^vars.β) for j in 1:n]
...
function generate_s(inner::ACOKInner, vars::ACOKSettings, i)
	...
	for i in 2:vars.k
		points[i] = sample(calculate_probabilities(inner, points, i, vars))
		for j in 1:(i-1)
...
function ACOK(graph, vars::ACOKSettings, η, τ)
	...
	for i in 1:vars.max_number_of_iterations
		...
		if i < 50
			S = Folds.map(_ -> randperm(n)[1:vars.k], zeros(vars.number_of_ants))
		else
			S = Folds.map(_ -> generate_s(inner, vars, rand(1:inner.n)), zeros(vars.number_of_ants))
		end
